...
<p style="font-size:12px;color:#9CA3AF;">© {{year}} ${BRAND.companyName}.com • {{companyAddress}}</p>
`;
...
const welcome = {
  ...
  html: wrap(`
    <h1 style="margin:0 0 8px;color:#111827;">Welcome, {{name}} 👋</h1>
    <p style="margin:0 0 16px;color:#374151;">
      You're set up with <strong>{{companyName}}</strong>. Start in your dashboard to finish intake and lock in your R&D credit estimate.
    </p>
    <p>
...
const docsReady = {
  name: 'Docs Ready',
  subject: 'Your R&D documents are ready for {{taxYear}} — download link inside',
  html: wrap(`
...
const receipt = {
  name: 'Receipt',
  subject: 'Payment received: ${{amount}} — {{tierName}}',
  html: wrap(`
    <h1 style="margin:0 0 8px;color:#111827;">Thanks for your payment</h1>
    <p style="margin:0 0 8px;color:#374151;">Amount: <strong>$` + `{{amount}}</strong> • {{tierName}}</p>
    <p style="margin:0 0 16px;color:#6B7280;">Paid on {{paymentDate}} • Card ending {{last4}}</p>
